...
export default function ConsultationPage() {
    ...
    <div>
      <div className="mx-auto mb-6 max-w-2xl text-center">
        ...
        <p className="mt-2 text-[12px] leading-5 text-[#6e7f94]">
          ...
          trajectory. Our 15-minute session helps clarify goals and next steps.
        </p>
      </div>
      ...
      <div className="mb-4 flex items-center justify-between rounded-[14px] bg-[#48e6b2] px-4 py-3">
        ...
        <button className="rounded-[10px] bg-[#0f3f38] px-4 py-2 text-[11px] font-semibold text-white">
          Join Live Queue
        </button>
      </div>
  ...
  <span key={`${day}-${index}`} className="font-medium text-[#8a9aad]">
    {day}
      ...
      <div className="mt-4 rounded-[16px] border border-dashed border-[#cddced] bg-[#f4f8ff] p-4">
        <p className="text-[11px] font-semibold uppercase text-[#1f6fd1]">
          Post-session Roadmap Preview
        </p>

        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <Step number="1" title="Needs Assessment" detail="Define academic hurdles" />
...
function Step({
  ...
  return (
    <div className="text-center">
      <div className="mx-auto flex h-6 w-6 items-center justify-center rounded-full bg-[#0b5dcc] text-[10px] font-semibold text-white">
        {number}
      </div>
